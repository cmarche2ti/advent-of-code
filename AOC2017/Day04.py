...
puzzle = """kvvfl kvvfl olud wjqsqa olud frc
...
passphrase = puzzle.split("\n")


def permute(s):
    tokens = s.split()
    tokens_sorted = [sorted(t) for t in tokens]
    tokens_sorted = ["".join(t) for t in tokens_sorted]
    print(len(tokens_sorted), len(set(tokens_sorted)))
    if len(tokens_sorted) == len(set(tokens_sorted)):
        return True
    else:
        return False
...
for p in passphrase:
    if permute(p):
        test_count = 0
        test_password = p.split()
        c = Counter(test_password).most_common()
        for value in c:
            test_count += value[1]
        print(test_count, len(c))
        if test_count == len(c):
            count += 1


print(len(passphrase), count)
